...
public:
	...
	bool IsBasePlayer()
	{
		...
		if (!Mem::IsReadable(pClassInfo->szName, 1))
			return false;
		if (hash_32_fnv1a_const(pClassInfo->szName) == hash_32_fnv1a_const("C_CSPlayerPawn"))
			return true;
		// Subclass / rename safety
		return std::strstr(pClassInfo->szName, "PlayerPawn") != nullptr;
	}
