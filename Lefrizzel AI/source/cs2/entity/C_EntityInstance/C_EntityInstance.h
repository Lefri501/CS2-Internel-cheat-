...
public:
	...
	{
		// Freed entities keep a dangling identity in user-range. Range-check
		// before index/flags; callers that need AV safety should wrap in SEH
		// (see CGameEntitySystem::Get).
		if (!this)
			return CBaseHandle();
		CEntityIdentity* identity = m_pEntityIdentity();
		if (!identity)
			return CBaseHandle();
		const auto ida = reinterpret_cast<std::uintptr_t>(identity);
