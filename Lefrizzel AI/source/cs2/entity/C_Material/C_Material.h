...
// scenesystem c_mesh_draw_primitive — IDA scenesystem.dll sub_1800545D0
// (CAnimatableSceneObjectDescRender / DrawArray):
//   loop: add rbx, 70h  →  stride 0x70 (NOT 0x68; old dump was stale)
//   +0x18 scene animatable, +0x20 material
//   +0x50 tint RGBA, +0x54 alpha scale
//   +0x62/+0x64 render flags
class CMeshData
{
public:
	// IDA verified 2026-07-17: three loop sites all do `add reg, 0x70`
	static constexpr std::size_t kStride = 0x70;
	...
	// Keep as opaque pad + known fields — do not invent named fields at 0x00
	// that could mislead future writes. Layout matches IDA stride 0x70.
	...
	Color color;                                   // 0x50
	float alphaScale;                              // 0x54
	std::uint8_t pad2[0x18];                       // 0x58 → 0x70 (flags @ 0x62/0x64 live here)
};

static_assert(sizeof(CMeshData) == 0x70, "mesh primitive stride");
