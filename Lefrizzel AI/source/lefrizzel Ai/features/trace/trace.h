...
// CGameTrace — IDA TraceShape 0x1809CD1E0 + InitTraceInfo 0x181640810:
//   +0x00 surface, +0x08 hit_entity, +0x10 hitbox (invalid_hitbox singleton often)
//   +0x78 startpos, +0x84 endpos, +0x90 normal
//   +0xAC fraction, +0xBB startsolid  (DidHit: frac<1 || startsolid)
// Prefer entity + Bones capsule over hitbox ptr for hit ID.
struct CGameTrace {
	...
	// CHitBox: +0x38 group, +0x48 index. invalid_hitbox → -1
	int hitgroup() const {
		...
		if (g < 0 || g > 8)
			return -1;
		if (g == 0 && (!hit_entity() || fraction() >= 1.f))
			return -1;
		return g;
	}
...
// Hit entity may be pawn / weapon / attachment — owner-chain match.
