...
public:
	...
	// SEH-safe identity serial read (impl in .cpp — header must stay SEH-free)
	static bool MatchHandleSerial(void* entity, const CBaseHandle& expected) noexcept;
	...
	T* Get(const CBaseHandle hHandle)
	{
		if (!hHandle.valid())
			return nullptr;
		T* e = Get<T>(hHandle.index());
		if (!e)
			return nullptr;
		// Serial check via CEntityInstance (T may not declare handle())
		// Index alone can resolve a recycled entity after death/round.
		if (!MatchHandleSerial(e, hHandle))
			return nullptr;
		return e;
	}
