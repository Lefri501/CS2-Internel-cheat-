...
public:
	...
	// width < 0 → fill remaining content width (clamped so it cannot clip).
	// width >= 0 → fixed width, still clamped to remaining space on the line.
	void menuButton(bool& feature, float width = -1.f);

	// ImGui: Always / Hold / Toggle combo for this feature.
	// width < 0 → fill remaining; width >= 0 → fixed (clamped).
	void menuMode(bool& feature, float width = -1.f);
