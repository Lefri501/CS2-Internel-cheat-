...
// Patterns from cs2_patterns_raw.txt — verified IDA a2bd4708 (client.dll):
//   GETBONEPOSITIONBYNAME / LOOKUPBONE → 0x18090F080
//   C_BASEENTITY_GETHITBOXSET         → 0x1808FD2C0
//   CALCULATEWORLDSPACEBONES          → 0x180A495B0
//   GETTRANSFORMSFORHITBOXLIST        → 0x180A58CD0 (calls CalcBones 0x100)
...
// Spine 0-6 is stable across CT/T player models
bool ValidateBoneArray(uintptr_t ptr, const Vector_t& origin, float height, int boneCount) {
	if (!ptr || ptr < 0x10000)
		return false;

	const int maxIdx = (boneCount > 7) ? (std::min)(boneCount - 1, 128) : 6;
	if (maxIdx < 6)
		return false;

	Vector_t pelvis{}, neck{}, head{};
	if (!GetBonePos(ptr, 0, pelvis) || !GetBonePos(ptr, 6, head))
		return false;

	if ((pelvis - origin).Length() > 80.f)
		return false;

	const float dzHead = head.z - origin.z;
	if (dzHead < height * 0.90f || dzHead > height * 1.35f)
		return false;

	if (GetBonePos(ptr, 5, neck) && neck.z > head.z - 1.f)
		return false;
...
bool Init() {
	...
	// GETBONEPOSITIONBYNAME — same target as LOOKUPBONE / GETBONEIDBYNAME
	...
	// GETTRANSFORMSFORHITBOXLIST (hitbox path; calls CalcBones with 0x100)
	...
	// LOOKUPBONE is an E8 call into GetBonePositionByName — keep as alias pointer
	if (auto* hit = M::FindPattern("client", "E8 ? ? ? ? 48 8B 8D ? ? ? ? B3")) {
		g_eng.getBoneIdByName = M::GetAbsoluteAddress(hit, 0x1, 0x0);
	} else if (g_eng.lookupBone) {
		g_eng.getBoneIdByName = g_eng.lookupBone;
	}

	g_eng.resolved = (g_Lookup != nullptr);
...
bool ForceUpdateBones(const C_CSPlayerPawn* pawn, unsigned int flags) {
	if (!g_CalcBones || !Mem::ValidEntity(pawn))
		return false;
...
bool GetOrigin(const C_CSPlayerPawn* pawn, Vector_t& out) {
	...
	if (!sceneOff || !originOff)
		return false;
	void* node = nullptr;
	if (!Mem::ReadField(pawn, sceneOff, node) || !Mem::Valid(node, originOff + sizeof(Vector_t)))
		return false;
	if (!Mem::ReadField(node, originOff, out))
		return false;
	return IsValidPos(out);
}

static bool IsLocalPawn(const C_CSPlayerPawn* pawn) {
	if (!pawn || !H::oGetLocalPlayer)
		return false;
...
static Vector_t EyeFromAbsView(const C_CSPlayerPawn* pawn) {
	...
	if (!GetOrigin(pawn, origin))
		return {};
	const uint32_t viewOffset = SchemaFinder::Get(
		hash_32_fnv1a_const("C_BaseModelEntity->m_vecViewOffset"));
	if (!viewOffset)
		return {};
	Vector_t view{};
	if (!Mem::ReadField(pawn, viewOffset, view))
		return {};
...
static bool ShootPosFromWeaponHistory(const C_CSPlayerPawn* pawn, Vector_t& out) {
	...
	const uint32_t wpnSvcOff = SchemaFinder::Get(
...
bool GetBoneArray(const C_CSPlayerPawn* pawn, uintptr_t& boneArray, Vector_t& origin, float& height) {
	...
	// Hitbox-ready flags (GetTransformsForHitboxList uses 0x100; SetupBones uses 0x20)
	...
	uintptr_t ptr = 0;
	if (TryBonePtr(skeleton, SKEL_BONE_PTR, ptr)
		&& ValidateBoneArray(ptr, origin, height, boneCount > 0 ? boneCount : 128)) {
		boneArray = ptr;
		return true;
	}
	...
	// NOTE: CModelState's bone-array pointer is NOT a schema-registered field in
	// CS2 — it lives at a fixed offset (modelState + 0x80 = BONE_PTR_OFF), which
	// the kOffs fallback below already covers. The old SchemaFinder::Get(
	// "CModelState->m_pBoneArray") always resolved to 0 and logged an offset-miss
	// every frame per player (log spam). Removed; the hardcoded offsets handle it.
	...
	for (size_t i = 0; i < sizeof(kOffs) / sizeof(kOffs[0]); ++i) {
		if (!TryBonePtr(skeleton, modelStateOff + kOffs[i], ptr))
			continue;
		if (!ValidateBoneArray(ptr, origin, height, boneCount > 0 ? boneCount : 128))
			continue;
		boneArray = ptr;
		return true;
	}
	...
	const Vector_t& origin, float height, Map& out)
{
	...
	if ((pelvis - origin).Length() > 80.f)
		return false;
	if (head.z < origin.z + height * 0.85f)
		return false;
...
// Studio hitbox indices (CS2 player models)
enum StudioHb : int {
...
void StudioIndicesForHb(int hb, const int*& list, int& count) {
	...
	default:                 list = head;    count = 1; break;
	}
}
...
bool ReadHitboxSet(void* entity, uintptr_t& arrayOut, int& countOut) {
	...
	if (!Mem::ReadField(set, kSetArray, arr) || !Mem::IsUserPtr(reinterpret_cast<void*>(arr)))
		return false;
	if (!Mem::IsReadable(reinterpret_cast<void*>(arr), kHitboxStride))
		return false;
	arrayOut = arr;
	countOut = count;
	return true;
...
bool GetHitboxCapsule(const C_CSPlayerPawn* pawn, int hb, Capsule& out) {
	...
	uintptr_t hbArr = 0;
	int hbCount = 0;
	if (!ReadHitboxSet(const_cast<C_CSPlayerPawn*>(pawn), hbArr, hbCount))
		return false;
	...
	const int* list = nullptr;
	int nList = 0;
	StudioIndicesForHb(hb, list, nList);
...
int StudioIndexToHitbox(int studioIndex) {
	switch (studioIndex) {
	case ST_HEAD: return Config::HB_HEAD;
	case ST_NECK: return Config::HB_NECK;
	case ST_PELVIS: return Config::HB_PELVIS;
	case ST_STOMACH: return Config::HB_STOMACH;
	case ST_CHEST:
	case ST_LOWER_CHEST:
	case ST_UPPER_CHEST: return Config::HB_CHEST;
	case ST_R_THIGH:
	case ST_L_THIGH:
	case ST_R_CALF:
	case ST_L_CALF: return Config::HB_LEGS;
	case ST_R_FOOT:
	case ST_L_FOOT: return Config::HB_FEET;
	case ST_R_HAND:
	case ST_L_HAND:
	case ST_R_UPPER_ARM:
	case ST_R_FOREARM:
	case ST_L_UPPER_ARM:
	case ST_L_FOREARM: return Config::HB_ARMS;
	default: return -1;
	}
}
	...
	float centerBias)
{
	...
	constexpr float kMaxDist = 8192.f;
	const Vector_t rayEnd{
		eye.x + dir.x * kMaxDist,
		eye.y + dir.y * kMaxDist,
		eye.z + dir.z * kMaxDist
	};

	const Vector_t u = rayEnd - eye;
	const Vector_t v = maxs - mins;
	const Vector_t w = eye - mins;
