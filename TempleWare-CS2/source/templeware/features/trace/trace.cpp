...
constexpr int kAvDisableThreshold = 8;
...
void NoteTraceOk() {
	...
	// Re-enable after a clean call if we soft-disabled earlier
...
// Match FireBullet wrapper filter (IDA sub_180231920) byte layout after InitFilter:
//   +8  mask qword
//   +50/-65536, +54/0x0F00  (wrapper magic — InitFilter uses +52=0x0F00FFFF)
//   +56 layer already set
//   +64 multi-hit = 0  (FireBullet single-hit; multi never returned world in logs)
void NormalizeFilter(std::uint8_t* filter, std::uint64_t mask) {
	*reinterpret_cast<std::uint64_t*>(filter + 8) = mask;
	// FireBullet: v25=-65536 @+50, v26=3840 @+54
	*reinterpret_cast<std::int32_t*>(filter + 0x32) = -65536;
	*reinterpret_cast<std::int16_t*>(filter + 0x36) = 3840;
	filter[0x40] = 0; // single-hit like FireBullet
}
	...
	for (int depth = 0; depth < 6 && cur && Mem::ValidEntity(cur); ++depth) {
		...
		__try {
			owner = *reinterpret_cast<CBaseHandle*>(
				reinterpret_cast<std::uint8_t*>(cur) + kOwnerEntityOff);
		} __except (EXCEPTION_EXECUTE_HANDLER) {
			break;
		}
...
bool PrepareAndCall(const Ray_t* ray, const Vector_t* start, const Vector_t* end,
	...
	if (!g_ready)
		return false;
	// Soft-disable: still try occasionally so a transient AV doesn't kill vis forever
	if (g_traceDisabled.load(std::memory_order_relaxed)) {
		static std::atomic<int> s_probe{0};
		if ((s_probe.fetch_add(1) % 64) != 0)
			return false;
	}
