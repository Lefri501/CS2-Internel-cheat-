...
bool Keybind::isActive() const {
	...
	case KeyMode::Hold:
		if (key == 0)
			return false;
		// Live high-bit read so hold tracks physical mouse/keyboard every tick
...
void Keybinds::menuMode(bool& feature) {
	...
	// Use full remaining width (menu sets width before call)
	if (ImGui::GetContentRegionAvail().x > 40.f)
	...
	if (ImGui::IsItemHovered())
		ImGui::SetTooltip("Always: on while enabled\nHold: only while key down\nToggle: key flips active");
	ImGui::PopID();
}

void Keybinds::menuButton(bool& feature) {
	Keybind* kb = find(feature);
	if (!kb) {
		ImGui::TextDisabled("N/A");
		return;
	...
	if (!kb->isListening) {
		...
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip("Press a key or mouse button (Esc cancel)");
